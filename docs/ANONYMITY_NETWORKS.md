# Anonymity Networks with vindex

Currently only Tor and I2P have been integrated into vindex. The usage of
...
When the `--proxy` flag is used, `vindexd` sends all external traffic via the
...
is made or until `vindexd` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `vindex-wallet-cli` and
`vindex-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
vindexd does not support synchronizing the blockchain over onion or I2P hidden services.
You may sync the blockchain using a SOCKS4 proxy. vindexd will connect to IPv4
...
`--tx-proxy` which tells `vindexd` the ip/port of a socks proxy provided by a
...
which tells `vindexd` to connect to ".onion" P2P addresses using a socks
...
`--anonymous-inbound`. This option tells `vindexd` the inbound address, network
...
which tells `vindexd` that a max of 25 inbound Tor connections are being
received at address "5tymba6faziy36md5ffy42vatbjzlye4vyr3gyz6lcvdfximnvwpmwqd.onion:18084" and forwarded to `vindexd`
...
forwarded to `vindexd` localhost port 18085. Using `tx-proxy`(required), these
...
`vindexd` RPC port - which is independent from the configuration for incoming
...
```text
HiddenServiceDir /var/lib/tor/data/vindex-rpc
...
```text
# P2P Hidden service
HiddenServiceDir /var/lib/tor/data/vindex
...
This will store key information in `/var/lib/tor/data/vindex` and `/var/lib/tor/data/vindex-rpc`
and will forward "Tor port" 18084 and 18089 to ports 18084 and 18089 of ip 127.0.0.1, respectively. The file
`/usr/lib/tor/data/vindex/hostname` will contain the ".onion" address for use with `--anonymous-inbound`, and
`/var/lib/tor/data/vindex-rpc/hostname` will contain the ".onion" address for use with RPC.

I2P must be configured with a standard server tunnel. Configuration differs by
I2P implementation.  
You can find guides for i2pd [here](https://docs.getvindex.org/running-node/vindexd-tori2p/#__tabbed_1_2).

## Example Node Configurations

Please check the [`vindexd` reference](https://docs.getvindex.org/interacting/vindexd-reference/#tori2p-and-proxies) for more information about these flags.
...
vindexd will connect to IPv4 nodes via clearnet, revealing to your ISP
that you are running a vindex node, but your transactions will be relayed over
Tor.

```bash
sudo apt install tor # Or install Tor some other way
systemctl start tor # Or start Tor manually
vindexd --tx-proxy tor,127.0.0.1:9050,10
```

### Connect To IPv4 Nodes Over Tor Only

This configuration does not connect to hidden services or accept incoming
connections. Your ISP will see that you are running Tor, but not vindexd.
...
You will need to configure [hidden services manually for Tor and I2P](https://docs.getvindex.org/running-node/vindexd-tori2p/#node-configuration).
...
This configuration is not currently supported. vindexd relies on IPv4 to sync
...
system clock is fairly accurate - many peers on the vindex network should have
similar timestamps.
...
### Intermittent vindex Syncing

If a user only runs `vindexd` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `vindexd` as often as possible to conceal when transactions are being sent.
...
`vindexd` currently selects two outgoing connections every 5 minutes for
...
`vindexd` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `vindexd` should likely disconnect
