# The Current/Future Status of ZMQ in vindex

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. vindex
...
matching supported by both vindex and ZMQ. The `format`, `context` and `event`
...
The vindex daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The vindex daemon also ensures that
...
The vindex daemon will send a `txpool_add` pub exactly once for each
