...
# a main offline vindexd, does most of the tests
# a restricted RPC vindexd setup with RPC payment
# two local online vindexds connected to each other
N_vindexDS = 5

# 4 wallets connected to the main offline vindexd
# 1 wallet connected to the first local online vindexd
# 1 offline wallet
N_WALLETS = 7
...
vindexd_base = [builddir + "/bin/vindexd", "--regtest", "--fixed-difficulty", str(DIFFICULTY), "--p2p-bind-port", "vindexd_p2p_port", "--rpc-bind-port", "vindexd_rpc_port", "--zmq-rpc-bind-port", "vindexd_zmq_port", "--zmq-pub", "vindexd_zmq_pub", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--data-dir", "vindexd_data_dir", "--log-level", "1", "--rpc-max-connections-per-private-ip", "100", "--rpc-max-connections", "100"]
...
wallet_base = [builddir + "/bin/vindex-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--log-level", "1", "--allow-mismatched-daemon-version"]
wallet_extra = [
...
for i in range(N_vindexDS):
  command_lines.append([str(18180+i) if x == "vindexd_rpc_port" else str(18280+i) if x == "vindexd_p2p_port" else str(18380+i) if x == "vindexd_zmq_port" else "tcp://127.0.0.1:" + str(18480+i) if x == "vindexd_zmq_pub" else builddir + "/functional-tests-directory/vindexd" + str(i) if x == "vindexd_data_dir" else x for x in vindexd_base])
  if i < len(vindexd_extra):
    command_lines[-1] += vindexd_extra[i]
  outputs.append(open(FUNCTIONAL_TESTS_DIRECTORY + '/vindexd' + str(i) + '.log', 'a+'))
