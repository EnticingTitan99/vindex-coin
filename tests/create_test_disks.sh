...
tmpdir=$(mktemp --tmpdir -d vindextest.XXXXXXXX)
...
echo vindex_TEST_DEVICE_HDD=$(device_mountpoint "$tmpdir" "$dev_rot")

dev_ssd=$(create_device "$tmpdir")
bdev_ssd=$(block_device_path "$dev_ssd")
echo 0 | root_exec tee "$bdev_ssd/queue/rotational" >/dev/null
echo vindex_TEST_DEVICE_SSD=$(device_mountpoint "$tmpdir" "$dev_ssd")
