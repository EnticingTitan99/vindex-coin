...
# Copyright (c) 2022-2024 The vindex Project
...
# target, so we don't spend time building vindex binaries.
...
GIT_ARCHIVE="${DIST_ARCHIVE_BASE}/vindex-source-${VERSION}.tar.gz"
...
if [ ! -e "$GIT_ARCHIVE" ]; then
    ...
    | sort \
    | tar --create --transform "s,^,vindex-source-${VERSION}/," --mode='u+rw,go+r-w,a+X' --files-from=- \
    | gzip -9n > ${GIT_ARCHIVE}
...
(
    ...
    # Setup the directory where our vindex build for HOST will be
