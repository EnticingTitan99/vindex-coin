...
if test -z "$vindexd"
then
  echo "vindexd not found"
  exit 1
fi
echo "Found: $vindexd"

TORDIR="$DIR/vindex-over-tor"
TORRC="$TORDIR/torrc"
...
"$vindexd" \
  ...
  --add-priority-node 2vdxnode5itf65lz.onion:18083 \
  --detach
ready=0
for i in `seq 10`
do
  sleep 1
  status=$("$vindexd" status)
...
if test "$ready" = 0
then
  echo "Error starting vindexd"
  tail -n 400 "$HOME/.bitvindex/bitvindex.log" | grep -Ev stacktrace\|"Error: Couldn't connect to daemon:"\|"src/daemon/main.cpp:.*vindex\ \'" | tail -n 20
  exit 1
fi
